...
const authMiddleware = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    try {
      ...
      // ≡ƒöÆ Single-device session enforcement
    ...
    console.error('Γ¥î Auth middleware error:', error);
...
const checkAndResetDailyCoins = async (userId: string) => {
  try {
    ...
    if (updatedUser && updatedUser.lastCoinClaim) {
      ...
      console.log('≡ƒöä Daily coin check:', {
        userId,
    ...
    console.error('Γ¥î Auto-reset coins error:', error);
    return null;
  }
};
...
router.post('/register', async (req, res) => {
  try {
    ...
    console.log('\n=== ≡ƒô¥ REGISTRATION ATTEMPT START ===');
    console.log('≡ƒôº Email:', email);
    console.log('≡ƒæñ Username:', username);
    console.log('ΓÅ░ Timestamp:', new Date().toISOString());
    console.log('≡ƒîÉ IP:', req.ip);
    console.log('≡ƒô▒ User Agent:', req.headers['user-agent']);

    // Validation
    if (!email || !password || !username) {
      console.log('Γ¥î FAILED: Missing required fields');
    ...
    console.log('Γ£à Validating gender selection...');
    if (!normalizedGender || !validGenders.has(normalizedGender)) {
      console.log('Γ¥î FAILED: Invalid or missing gender selection');
      console.log('=== REGISTRATION ATTEMPT END ===\n');
      return res.status(400).json({
        success: false,
    ...
    console.log('Γ£à Gender selection valid:', normalizedGender);

    const normalizedEmail = email.trim().toLowerCase();

    // Email validation
    console.log('Γ£à Validating email format...');
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!emailRegex.test(normalizedEmail)) {
      console.log('Γ¥î FAILED: Invalid email format');
      console.log('=== REGISTRATION ATTEMPT END ===\n');
      return res.status(400).json({
        success: false,
    ...
    console.log('Γ£à Email format valid');

    const emailDomain = normalizedEmail.split('@')[1];
    if (!TRUSTED_EMAIL_PROVIDERS.has(emailDomain)) {
      console.log('Γ¥î FAILED: Unsupported email domain', emailDomain);
      console.log('=== REGISTRATION ATTEMPT END ===\n');
      return res.status(400).json({
        success: false,
    ...
    console.log('Γ£à Email domain allowed:', emailDomain);

    // Password validation (min 6 characters)
    console.log('Γ£à Validating password...');
    if (password.length < 6) {
      console.log('Γ¥î FAILED: Password too short');
      console.log('=== REGISTRATION ATTEMPT END ===\n');
      return res.status(400).json({
        success: false,
    ...
    console.log('Γ£à Password valid');

    // Username validation
    console.log('Γ£à Validating username...');
    if (username.length < 3) {
      console.log('Γ¥î FAILED: Username too short');
      console.log('=== REGISTRATION ATTEMPT END ===\n');
      return res.status(400).json({
        success: false,
    ...
    console.log('Γ£à Username valid');

    // Check if user already exists
    console.log('≡ƒöì Checking if email already registered...');
    const existingUser = await DatabaseService.getUserByEmail(normalizedEmail);
    if (existingUser) {
      if (existingUser.isVerified) {
        console.log('Γ¥î FAILED: Email already registered (verified account):', normalizedEmail);
      ...
      console.log('≡ƒº╣ Legacy unverified user found. Removing stale database record before pending registration.');
    ...
    console.log('Γ£à No verified user found for this email. Proceeding with pending registration.');

    // Hash password for storage (kept in Redis until verification succeeds)
    console.log('≡ƒöÉ Hashing password...');
    const passwordHash = await bcrypt.hash(password, 10);
    console.log('Γ£à Password hashed successfully');

    // ≡ƒôº Generate OTP for email verification
    console.log('≡ƒôº Generating OTP...');
    const otp = generateOTP();
    const otpExpiresAt = new Date(Date.now() + 10 * 60 * 1000); // 10 minutes
    console.log('Γ£à OTP generated:', otp, '(expires at:', otpExpiresAt.toLocaleTimeString(), ')');
    ...
    console.log('≡ƒÆ╛ Storing pending registration in Redis:', {
      key: pendingKey,
      email: normalizedEmail,
      username,
    ...
    // ≡ƒôº Send OTP email
    console.log('≡ƒôº Sending OTP email...');
    ...
    if (!emailSent) {
      console.warn('ΓÜá∩╕Å  Failed to send OTP email (pending registration stored).');
    } else {
      console.log('Γ£à OTP email sent successfully');
    }
    ...
    console.log('≡ƒÄë REGISTRATION PENDING (OTP sent, user not yet persisted):', {
      email: normalizedEmail,
      username,
      tokenProvided: false,
    ...
    console.error('Γ¥î CRITICAL REGISTRATION ERROR:', error);
...
router.post('/verify-otp', async (req, res) => {
  try {
    ...
    console.log('\n=== ≡ƒôº OTP VERIFICATION START ===');
    console.log('≡ƒôº Email:', email);
    console.log('≡ƒöó OTP:', otp);
    console.log('ΓÅ░ Timestamp:', new Date().toISOString());

    // Validation
    if (!email || !otp) {
      console.log('Γ¥î FAILED: Missing email or OTP');
      return res.status(400).json({
        success: false,
    ...
    if (otp.length !== 6) {
      console.log('Γ¥î FAILED: Invalid OTP format');
      return res.status(400).json({
        success: false,
    ...
    if (pendingRegistration) {
      ...
      if (expiryDate && now > expiryDate) {
        console.log('Γ¥î FAILED: Pending registration OTP expired');
      ...
      if (pendingRegistration.otp !== otp) {
        console.log('Γ¥î FAILED: Invalid OTP for pending registration');
      ...
      // OTP matches ΓÇô create the user now
      console.log('Γ£à OTP matches pending registration. Creating verified user...');
      ...
      console.log('Γ£à User persisted after OTP verification:', {
        id: createdUser.id,
      ...
      console.log('≡ƒÄë OTP VERIFIED & USER CREATED (pending flow)');
      console.log('=== OTP VERIFICATION END ===\n');

      return res.json({
        success: true,
        message: 'Email verified successfully! Welcome to Omegoo ≡ƒÄë',
        token,
        user: {
          id: createdUser.id,
          email: createdUser.email,
    ...
    if (!user) {
      console.log('Γ¥î FAILED: User not found (no pending registration either)');
    ...
    if (user.isVerified) {
      console.log('Γ£à User already verified');
      return res.json({
        success: true,
    ...
    if (user.otpExpiresAt) {
      ...
      console.warn('ΓÜá∩╕Å No OTP expiry set on user (legacy). Allowing verification if OTP matches.');
    }
    if (expiryDebug) console.log('≡ƒòÉ OTP Expiry Check:', expiryDebug);
    if (isExpired) {
      console.log('Γ¥î FAILED: OTP expired');
      return res.status(400).json({
        success: false,
    ...
    if (user.otp !== otp) {
      console.log('Γ¥î FAILED: Invalid OTP');
    ...
    console.log('≡ƒÄë OTP VERIFIED SUCCESSFULLY (legacy user)');
    console.log('=== OTP VERIFICATION END ===\n');
    ...
    console.error('Γ¥î OTP VERIFICATION ERROR:', error);
    res.status(500).json({
      success: false,
...
router.post('/resend-otp', async (req, res) => {
  try {
    ...
    if (pendingRegistration) {
      ...
      console.log('Γ£à OTP resent successfully for pending registration');
    ...
    console.log('Γ£à OTP resent successfully (legacy user)');
    console.log('=== RESEND OTP END ===\n');
    ...
    console.error('Γ¥î RESEND OTP ERROR:', error);
    res.status(500).json({
      success: false,
...
router.post('/login', async (req, res) => {
  try {
    ...
    // Validation
    if (!email || !password) {
      console.log('Γ¥î FAILED: Missing credentials');
    ...
    if (!emailAllowed || !ipAllowed) {
      console.log('≡ƒÜ½ LOGIN RATE LIMIT TRIGGERED:', {
        email: normalizedEmail,
        clientIp,
        emailAllowed,
        ipAllowed
    ...
    // Find user by email
    console.log('≡ƒöì Looking up user in database...');
    const user = await DatabaseService.getUserByEmail(normalizedEmail);
    
    if (!user) {
      console.log('Γ¥î FAILED: User not found for email:', email);
      console.log('=== LOGIN ATTEMPT END ===\n');
      return res.status(401).json({
        success: false,
    ...
    if (!user.passwordHash) {
      console.log('Γ¥î FAILED: OAuth user trying email login:', email);
      console.log('=== LOGIN ATTEMPT END ===\n');
      return res.status(400).json({
        success: false,
    ...
    // Verify password
    console.log('≡ƒöæ Verifying password...');
    const isValidPassword = await bcrypt.compare(password, user.passwordHash);
    
    if (!isValidPassword) {
      console.log('Γ¥î FAILED: Invalid password for user:', email);
      console.log('=== LOGIN ATTEMPT END ===\n');
      return res.status(401).json({
        success: false,
        error: 'Invalid email or password'
    ...
    console.log('Γ£à Password verified successfully');

    // Check if user is banned
    console.log('≡ƒÜ½ Checking ban status...');
    const banStatus = await DatabaseService.checkUserBanStatus(user.id);
    if (banStatus) {
      console.log('Γ¥î FAILED: User is banned:', {
        userId: user.id,
    ...
    console.log('Γ£à User not banned');

    // Require email verification before issuing token
    if (!user.isVerified) {
      console.log('Γ¥î FAILED: Email not verified. Blocking login until OTP verified.');
      console.log('=== LOGIN ATTEMPT END ===\n');
      return res.status(403).json({
        success: false,
    ...
    console.log('≡ƒ¬Ö Checking daily coin reset...');
    const updatedUser = await checkAndResetDailyCoins(user.id);
    const finalUser = updatedUser || user;
    console.log('Γ£à Coin check complete. Current coins:', finalUser.coins);

    // Generate JWT token
    console.log('≡ƒÄƒ∩╕Å  Generating JWT token...');
    const token = jwt.sign(
      { userId: finalUser.id, email: finalUser.email, role: finalUser.tier },
      process.env.JWT_SECRET as string,
      { expiresIn: '7d' }
    );
    console.log('Γ£à Token generated:', token.substring(0, 20) + '...');

    // ≡ƒöÆ Save token to enforce single-device session
    const userAgent = req.headers['user-agent'] || 'Unknown Device';
    const deviceInfo = `${userAgent.substring(0, 50)} - ${new Date().toLocaleString()}`;

    console.log('≡ƒÆ╛ Saving session to database...');
    console.log('≡ƒöÆ Previous activeDeviceToken:', finalUser.activeDeviceToken ? finalUser.activeDeviceToken.substring(0, 20) + '...' : 'None');
    console.log('≡ƒöÆ New activeDeviceToken:', token.substring(0, 20) + '...');
    console.log('≡ƒô▒ Device info:', deviceInfo);

    await DatabaseService.updateUser(finalUser.id, {
      lastActiveAt: new Date(),
      activeDeviceToken: token, // ≡ƒöÆ This will REPLACE old token - previous sessions will be invalidated
      lastLoginDevice: deviceInfo
    });

    console.log('Γ£à Session saved successfully');
    console.log('≡ƒÄë LOGIN SUCCESSFUL:', { 
      userId: finalUser.id, 
    ...
    try {
      ...
      console.warn('ΓÜá∩╕Å Failed to clear login rate limit counters:', clearError);
    }
    ...
    console.error('Γ¥î CRITICAL LOGIN ERROR:', error);
    console.error('Stack trace:', error.stack);
...
router.post('/google', async (req, res) => {
  try {
    ...
    try {
      ...
      console.error('Γ¥î Failed to decode Google token:', decodeError);
      return res.status(400).json({
        success: false,
    ...
    console.log('≡ƒöÉ Google OAuth attempt:', { email, name, googleId });
    ...
    if (!user) {
      ...
      console.log('≡ƒåò Creating new Google OAuth user:', email);
      
      user = await DatabaseService.createUser({
        email,
        username: autoUsername, // Auto-generated username from email (before @)
        passwordHash: undefined, // No password for OAuth users
        tier: 'verified', // Γ£à Google accounts are pre-verified ΓåÆ verified tier
        status: 'active',
        isVerified: true, // Γ£à Google accounts are pre-verified
        coins: 50,
        totalChats: 0,
        dailyChats: 0,
        lastCoinClaim: new Date(),
      ...
      console.log('Γ£à Google OAuth user created:', { id: user.id, email: user.email, username: user.username });

      // ≡ƒôº Send welcome email (no OTP needed for Google users)
      console.log('≡ƒôº Sending welcome email to Google user...');
      ...
      if (emailSent) {
        console.log('Γ£à Welcome email sent successfully');
      } else {
        console.warn('ΓÜá∩╕Å  Failed to send welcome email');
      }
    } else {
      // Check if user is banned
      const banStatus = await DatabaseService.checkUserBanStatus(user.id);
    ...
    // ≡ƒöÆ Save token for single-device session
    const userAgent = req.headers['user-agent'] || 'Unknown Device';
    const deviceInfo = `${userAgent.substring(0, 50)} - ${new Date().toLocaleString()}`;
    ...
    console.log('Γ£à Google OAuth successful:', { userId: user.id, email: user.email, device: deviceInfo });
...
router.post('/request-otp', async (req, res) => {
  try {
    ...
    // SMS integration will be added when phone verification is implemented
...
router.get('/me', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    console.log('≡ƒæñ Fetching user data:', { userId: req.userId });
    ...
    console.log('Γ£à User data fetched:', { 
      userId: user.id, 
      email: user.email, 
      username: user.username,
      coins: user.coins,
    ...
    console.error('Γ¥î Get current user error:', error);
    res.status(500).json({
      success: false,
...
router.post('/forgot-password', async (req: Request, res: Response) => {
  try {
    ...
    if (user) {
      ...
      if (!emailSent) {
        console.warn('ΓÜá∩╕Å Password reset email dispatch failed', { email: normalizedEmail });
      }
    } else {
      console.log('Γä╣∩╕Å Password reset requested for non-existent email', { email: normalizedEmail });
    }
...
router.post('/validate-reset-token', async (req: Request, res: Response) => {
  try {
    ...
    console.error('Γ¥î Validate reset token error:', error);
    return res.status(500).json({
      success: false,
...
router.post('/reset-password', async (req: Request, res: Response) => {
  try {
    ...
    console.error('Γ¥î Reset password error:', error);
    return res.status(500).json({
      success: false,
...
router.post('/change-password', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    console.log('Γ£à Password changed for user:', userId);
    ...
    console.error('Γ¥î Change password error:', error);
    res.status(500).json({
      success: false,
